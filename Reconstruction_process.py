...
from AVAT import AVAT_DPA_Block
from networkUtil import dataConsistencyLayer
from function import adaptive_instance_normalization


parser = ArgumentParser(description='ATLF')

parser.add_argument('--start_epoch', type=int, default=0, help='epoch number of start training')
parser.add_argument('--end_epoch', type=int, default=200, help='epoch number of end training')
parser.add_argument('--layer_num', type=int, default=9, help='phase number of ATLF')
...
class BasicBlock(torch.nn.Module):
    def __init__(self):
        ...
        self.DC=dataConsistencyLayer(0)
        ...
        self.AdaIN =adaptive_instance_normalization(content_feat, style_feat)
        self.conv_G = nn.Parameter(init.xavier_normal_(torch.Tensor(1, 32, 3, 3)))
        self.AVAT=AVAT_DPA_Block()
    def forward(self, x,x_c, PhiTPhi, PhiTb):
        x = x - self.lambda_step * torch.mm(x, PhiTPhi)
        x = x + self.lambda_step * PhiTb
        x_input = x.view(-1, 1, 33, 33)

        x_f = F.conv2d(x_input, self.conv_D, padding=1)
        ...
        x_forward=DC(x_forward,x_f)
        x_forward=x_forward+x_input
...
class ATLFNetplus(torch.nn.Module):
    def __init__(self, LayerNo):
        super(ATLFNetplus, self).__init__()
        onelayer = []
...
model_dir = "" % (args.model_dir, layer_num, group_num, cs_ratio, learning_rate)

log_file_name = "" % (args.log_dir, layer_num, group_num, cs_ratio, learning_rate)
...
if start_epoch > 0:
    ...
    model.load_state_dict(torch.load('' % (pre_model_dir, start_epoch)))
...
for epoch_i in range(start_epoch+1, end_epoch+1):
    ...
    if epoch_i % 5 == 0:
        torch.save(model.state_dict(), "" % (model_dir, epoch_i))  # save only the parameters
